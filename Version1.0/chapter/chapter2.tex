...
\begin{figure}[!htb]
 ...
 \vspace{-0.4cm}
\end{figure}
紧组合方案中，可以将GNSS和SINS按组合的要求进行一体化设计，伪距/伪距率组合，以及在此基础上用SINS的位置和速度对接收机跟踪环路进行辅助，为有效利用各导航子系统的导航信息，提高精度与松组合相比具有更高的定位精度及输出稳定性，同时利用两种定位方式相互补充的特点，如图，伪距/伪距率组合模式是用GPS给出的星历数据和SINS给出的位置和速度计算出相应于SINS位置的伪距和伪距率，并将其与GPS测量的伪距和伪距率相比较作为量测值，通过Kalman滤波器估计SINS和GPS的误差量，然后对两个系统进行校正，由于GPS的测距容易建模，因而可以把它扩充为状态，通过滤波器加以估计，然后对GPS接收机进行校正。
