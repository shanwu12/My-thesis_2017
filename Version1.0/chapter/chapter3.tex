...
\section{引言}
根据前一章系统总体方案的设计，本章的重点工作是制作系统的硬件平台。本车载组合定位系统硬件设计分为：器件选型、嵌入式ARM系统设计、外围设备设计、数据传感器设计，硬件测试等。本文是利用已具备科研条件，在实验室的研究基础上，提出自己的解决方案：设计一个低成本、精度可接受的MEMS \ SINS/GNSS组合的小型化导航系统。整个硬件设计过程需考虑设计成本、硬件尺寸、产品功耗、抗干扰性能和产品实用性等问题。设计流程如下图\ref{chap3_Fig.1}所示：
\begin{figure}[!htb]
 \centering
...
\section{主要器件选型}
本课题是利用MIMU构建捷联惯性导航系统，然后接收卫星信号对导航结果进行补偿和融合，得到可信度高的姿态、速度和位置等信息，最终可以应用在地面车辆上，并具备良好的稳定性和抗干扰能力。所以，在设计时不仅需要对其进行规划分析，还要根据工作环境和运行条件进行芯片的选型。以下介绍几个主要器件的选型。
