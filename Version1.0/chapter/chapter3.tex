...
NEO-M8N有多种接口方式：UART、SPI以及USB接口三种模式；数据输出格式有：NMEA、UBX和RTCM。本文将NEO-M8N与STM32芯片相连，采用最为方便利于开发的USART接口连接方式，输出数据格式采用工程中常用的NMEA-0183协议。
...
\begin{figure}[!htb]
 ...
 \vspace{-0.4cm}
\end{figure}
电源是一个系统中及其重要的一部分，电源的设计直接影响到系统能否正常工作。根据设计要求，模块通过USB供电或者引出电源管脚外部5V供电，为了使整个系统稳定的工作，选择稳压芯片输出稳定的电压。系统中需要的电压有5V 和3.3V。因此本系统对电源做了两级处理，前级电源输出5V的电压，接着连接3.3V的稳压芯片，保证了系统供电的稳定性，还加入使用500mA的保险丝作为过流保护。本系统的输入输出压降教小、功率要求低，设计时选择德州仪器的线性电源芯片TPS73133如图\ref{chap3_Fig.5}所示 ，该芯片是具有反向电流保护的超低功耗电压稳压器件。为了增加安全性，还具有热关断、电流限制和反向电流保护功能，还可以通过拉低使能(EN)引脚将稳压器置于关断模式，这个模式的关断电流低至150nA （典型值）。
...
电路板设计加工完成后，首先进行硬件电路测试，调试没有问题后才可以进行系统软件测试。将主芯片通过JLINK调试器连接到电脑，使用JTAG调试代理软件，就可以检查到板上的芯片如图\ref{chap3_Fig.18}，说明硬件设计正常。
\begin{figure}[!htb]
 \centering
 ...
 \vspace{-0.4cm}
\end{figure}
编写一段测试程序，验证各模块正常工作，至此，硬件设计完成。检查是否可以长时间稳定工作。电子电路经静态和动态调试正常之后，便可对课题要求的技术指标进行测量，测试并记录数据，确定电路技术指标符合设计要求。

%\subsection{屏蔽罩外壳设计}
本课题针对系统板设计了屏蔽外壳，一方面可以保护屏蔽罩内的元器件，另一方面作用是屏蔽电磁干扰（EMI）。屏蔽就是对两个空间区域之间进行金属的隔离，以控制产品外电场、磁场和电磁波对设备的感应和辐射。本设计在AutoCAD 软件上根据物体实际尺寸绘制了屏蔽罩三维模型如图\ref{chap3_Fig.19}，其中外壳使用的材料是洋白铜（GPS屏蔽罩一般都用洋白铜，在众多材料中洋白铜的屏蔽效果十分出色，是一种容易上锡的金属屏蔽材料，且在焊接、散热方面表现比较好）。
\begin{figure}[htbp]
\begin{minipage}[t]{0.35\linewidth}
\centering
