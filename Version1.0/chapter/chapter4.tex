...
根据式(?)的MEMS惯导比力方程，若考虑系统中存在的各种误差。则MEMS惯导的速度计算应该表示为：
\begin{equation}
\boldsymbol{\delta\dot{V}}^n=-\varphi^n \times \boldsymbol{f}^b+\delta V^n\times(2\boldsymbol \omega^n_{ie}+\boldsymbol\omega^n_{en})+ \boldsymbol{V}^n\times(2\delta\boldsymbol{\omega}^n_{ie}+\delta\boldsymbol{\omega}^n_{en})+ab^n
\end{equation}
